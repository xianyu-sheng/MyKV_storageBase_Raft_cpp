...
为了系统地评估分布式 KV 存储在不同工作负载下的性能表现，项目实现了一套压测（Benchmarking）功能。通过模拟多并发场景和大量数据操作，可以收集以下关键指标：
...
1. **参数配置**
   - 通过命令行参数（如 `--bench`、`--ops`、`--threads`）指定压测的操作数、并发线程数等。
   - Clerk 根据参数初始化多个客户端线程，每个线程独立执行 Get / Put / Append 操作。

2. **并发执行**
   - 启用指定数量的工作线程（如 4、8、16），每个线程循环发送 RPC 请求。
   - 使用原子操作和线程安全的计数器跟踪已完成的操作数和失败数。

3. **性能数据采集**
   - 记录每次操作的执行时间（使用 `std::chrono::high_resolution_clock`）。
   - 定期输出进度和中间统计数据，便于实时监控压测状态。
...
```bash
# 单线程，执行 100 次操作（默认均匀分布 Get/Put/Append）
cd /home/your_user/MyKV_storageBase_Raft_cpp/build
./kvclient -i ../myRPC/conf/myrpc.conf -- --bench --ops 100
```

#### 多并发压测

```bash
# 4 个并发线程，各执行 100 次操作，总 400 个操作
...
```bash
# 可以扩展命令行参数支持特定操作（如只做 Put 或只做 Get）
# 示例伪代码：
# ./kvclient -i ../myRPC/conf/myrpc.conf -- --bench --ops 1000 --threads 8 --put-ratio 0.7 --get-ratio 0.3
```

### 性能表现评估

根据压测结果，可以从以下几个维度评估系统性能：

#### 1. 吞吐量

- **单线程吞吐量**：通常在 10–50 Ops/sec 量级（取决于网络延迟和 Raft 复制开销）。
- **多线程吞吐量**：随着线程数增加会有一定提升，但受限于 Leader 的处理能力和网络带宽。
  - 4 线程：可能达到 30–100 Ops/sec。
  - 8 线程及以上：增益开始饱和，因为主要瓶颈在 Raft 日志提交延迟（通常 25ms 心跳间隔）。

#### 2. 延迟

- **平均延迟**：通常在 50–200ms 范围。
  - 包含：RPC 网络往返（通常 1–5ms）+ Raft 复制延迟（多数派确认，通常 20–100ms）+ 本地 KV 操作（<1ms）。
- **P99 延迟**：偶发的网络抖动或 GC 可能导致尖峰，P99 通常是平均延迟的 2–5 倍。

> **注意**：当前版本中读操作也走 Raft 日志，因此读延迟与写延迟接近，不存在"读快写慢"的特性。

#### 3. 可靠性

- **成功率**：在正常网络条件下应接近 100%（>99.9%）。
- **常见失败原因**（可在压测日志中观察）：
  - `ErrWrongLeader`：客户端请求到了非 Leader 节点，通常会重试到 Leader（正常现象）。
  - 网络超时（`CONSENSUS_TIMEOUT`）：可能由于网络波动或 Leader 繁忙，客户端会重试。
  - 节点不可用：若某个节点宕机，其他节点应仍能对外服务（容错体现）。

### 压测结果示例与解读

根据上述压测过程，一个典型的运行结果如下：

```
========== Benchmark Result ==========
Total Operations: 400
Successful Operations: 398
Failed Operations: 2
Success Rate: 99.50%
Total Time: 45.231 seconds
Throughput: 8.84 Ops/sec
...
**解读**：
- **吞吐量 8.84 Ops/sec**：4 个线程，100 个操作/线程，45 秒完成，说明单个操作平均消耗约 50ms（包括 Raft 复制）。
- **成功率 99.5%**：2 个失败都是 `ErrWrongLeader`，说明客户端的重试机制正在工作（实际最终可能成功）。
- **延迟 P99 = 245.67ms**：说明大多数操作快速完成，但偶发操作可能因为网络波动或 Leader 繁忙而延迟较长。

### 性能优化建议

虽然当前压测效果在学习项目的范围内，但以下方向可以进一步优化：

1. **读性能优化**
   - 当前读也走 Raft 日志，导致读延迟与写相同。
   - 可以考虑实现 **ReadIndex** 或 **lease read** 机制，让读在 Leader 本地快速完成，性能可提升 10 倍以上。

2. **Raft 心跳优化**
   - 当前心跳间隔约 25ms，可以根据负载动态调整。
   - 减小心跳间隔可以降低平均延迟，但会增加网络开销。

3. **批量提交**
   - 当前每个客户端请求单独通过 Raft 提交。
   - 可以在 Leader 实现批量提交（Batching），将多个请求合并成一个 Raft 日志条目，大幅提升吞吐量。

4. **网络优化**
   - 启用 TCP Nagle 禁用、连接复用等优化手段，减少网络往返时间。

### 压测场景拓展

未来可以支持以下更多压测场景：

- **热点数据测试**：集中访问少数键，模拟缓存行为。
- **节点故障模拟**：在压测过程中 kill 某个节点，观察系统的容错行为和恢复时间。
- **长期稳定性测试**：运行数小时或数天，观察内存泄漏、日志堆积等问题。
- **分布式网络延迟模拟**：使用 `tc`（Traffic Control）工具在网络层引入延迟和丢包，模拟不稳定网络。
